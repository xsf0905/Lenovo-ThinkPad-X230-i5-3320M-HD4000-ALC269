function paddleAffiliateAnalyticsClient(stub) {
    this._apiBaseUrl = 'https://a.paddle.com/event/';
    ...
    this.action = {
        reservedActionProperties: {
            ...
            utmTerm: "$utm_term"
        }, _globalProperties: {}, _client: this, _hasTracked: false, track: function (action, params, blocking) {
    ...
    this.user = {
        reservedProfileProperties: {
            ...
            phone: "$phone",
            sms: "$sms"
        }, _client: this, _anon: true, identify: function (identity) {
            ...
            var payload = {old_actor: oldActor, new_actor: newActor, write_key: this._client._writeKey};
            ...
            var payload = {write_key: this._client._writeKey, actor: this._client.actor, properties: params};
    ...
    this._util = {
        _mergeObjects: function (target, source) {
            ...
            var os = unknown;
            var clientStrings = [{s: 'Windows CE', r: /Windows CE/}, {s: 'Windows 3.11', r: /Win16/}, {
                s: 'Windows 95',
            ...
            }, {s: 'Windows 10', r: /(Windows 10|Windows NT 10)/}, {s: 'Android', r: /Android/}, {
                s: 'Open BSD',
                r: /OpenBSD/
    ...
    this._ajaxPost = function (url, params, success, error, blocking) {
        if (this._writeKey == null) {
            console.warn("paddleAffiliateAnalytics API method was called before init(...). Call was ignored.");
        } else {
            ...
            if (xmlHttp.readyState === 4) {
                ...
                if (xmlHttp.status === 200 && response != null && response.error == null) {
                    ...
                    if (xmlHttp.status === 403 && thisClosure._offlineQueue.isSupported()) {
                        console.log("Received 500 from paddleAffiliateAnalytics API server. Queuing for retry later.");
                        thisClosure._offlineQueue.enqueueApi(url, params);
                    }
                    else if (error != null) {
                        error.call(thisClosure, response);
                        console.log("Error reported from paddleAffiliateAnalytics API server", response);
                    }
                }
            } else {
            }
        ...
        if (!blocking) {
            ...
            xmlHttp.ontimeout = function () {
                if (thisClosure._offlineQueue.isSupported()) {
                    console.log("No response from paddleAffiliateAnalytics API server (timeout). Queuing for retry later.");
                    thisClosure._offlineQueue.enqueueApi(url, params);
                }
            }
        }
    ...
    this._jsonpPost = function (url, params, success, error) {
        ...
        el.src = url + "?data=" + this._util._Base64.encode(JSON.stringify(params)) + "&callback=paddleAffiliateAnalytics._jsonpCallback"
        el.async = !0;
    ...
    this._jsonpCallback = function (result) {
        if (!result || result.error != null) {
            throw("A paddleAffiliateAnalytics API call has failed." + (result != null ? " Err = " + result.error : ""));
        }
    };
    ...
    this._offlineQueue = {
        _storageKey: "analytics.paddle.com/offlineQueue", enqueueApi: function (url, apiParams) {
    ...
    this._overwriteStub = function (stub) {
        if (stub == null || stub.__SV == null) {
            throw("The paddleAffiliateAnalytics library was not initialised correctly. Please use the snippet provided on the paddleAffiliateAnalytics website.")
        }
        if (stub.__SV < 0) {
            console.warn("The paddleAffiliateAnalytics initialisation snippet is out of date. Please use the latest snippet from the paddleAffiliateAnalytics website.")
        }
        if (stub.writeKey == null) {
            this.init = function () {
                stub.init.apply(this, arguments);
                window.paddleAffiliateAnalytics = new paddleAffiliateAnalyticsClient(stub);
            }
        } else {
            if (this._writeKey != null) {
                console.warn("paddleAffiliateAnalytics has already been initialised. You should not call init(...) more than once.")
                return;
            }
            if (stub.writeKey == null || stub.writeKey.length != 32) {
                throw("Invalid key specified for paddleAffiliateAnalytics library.")
            }
            if (window.location.protocol === "file:") {
                console.warn("Source is offline. paddleAffiliateAnalytics will not process API calls.")
                return;
            }
...
window.paddleAffiliateAnalytics = new paddleAffiliateAnalyticsClient(window.paddleAffiliateAnalytics);